...
def spring_layout_embedding(
    ...
    Since the implementation of `nx.spring_layout` embeds edge weights in wᵢⱼ = (k/rᵢⱼ)^3, the
    algorithm is run with:
    - `k=1`: to ignore this parameter.
    - `scale=None`: no rescaling of the layout.
    - edge weights are rescaled by sqrt(weight) before embedding, to embed in wᵢⱼ=1/rᵢⱼ^6.
