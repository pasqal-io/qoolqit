...
def make_remote_job(
    ...
    calls = iter(statuses)
    connection._query_job_progress.side_effect = (
        lambda _batch_id: {job_id: next(calls)}
    )
...
class TestLocalJob:
    ...
    def test_job_id_is_empty_string(self):
...
class TestRemoteJob:
    ...
    def test_job_id_returns_correct_value(self):
        connection = MagicMock(spec=remote.RemoteConnection)
    ...
    def test_from_remote_results_selects_by_position(self):
        connection = MagicMock(spec=remote.RemoteConnection)
    ...
    def test_from_remote_results_defaults_to_last(self):
        connection = MagicMock(spec=remote.RemoteConnection)
        rr = MagicMock(spec=remote.RemoteResults, _connection=connection)
    ...
    def test_from_remote_results_raises_on_bad_index(self):
        connection = MagicMock(spec=remote.RemoteConnection)
        rr = MagicMock(spec=remote.RemoteResults, _connection=connection)
        rr.job_ids = []
        rr.batch_id = "b"
...
class TestRetrieveRemoteJob:
    def test_returns_remote_job_instance(self):
        connection = MagicMock(spec=remote.RemoteConnection)
        job = retrieve_remote_job(connection, "job-id", batch_id="batch-id")
        assert isinstance(job, _RemoteJob)

    def test_job_id_and_batch_id_are_set(self):
        connection = MagicMock(spec=remote.RemoteConnection)
        job = retrieve_remote_job(connection, "job-id", batch_id="batch-id")
        assert job.job_id() == "job-id"
        assert job._batch_id == "batch-id"

    def test_batch_id_defaults_to_empty(self):
        connection = MagicMock(spec=remote.RemoteConnection)
...
class TestGetBatchId:
    def test_returns_batch_id_for_remote_job(self):
        connection = MagicMock(spec=remote.RemoteConnection)
        job = _RemoteJob(connection, "j", batch_id="my-batch")
        assert get_batch_id(job) == "my-batch"

    def test_returns_empty_string_for_local_job(self):
