...
def test_drive_different_phase_composition() -> None:
    ...
    assert composite_drive._split_in_phase_groups() == [
...
def test_drive_phase_composition_with_padded_waveforms() -> None:
    ...
    group_1_amp, group_1_det, _ = composite_drive._split_in_phase_groups()[0]
    group_2_amp, group_2_det, _ = composite_drive._split_in_phase_groups()[1]
...
def test_drive_same_phase_composition(phase: float) -> None:
    ...
    assert len(composite_drive._split_in_phase_groups()) == 1
    group_amp, group_det, group_phase = composite_drive._split_in_phase_groups()[0]
...
def test_drive_chained_composition_merges_adjacent_same_phase() -> None:
    ...
    group_phases = [phase for _, _, phase in composite_drive._split_in_phase_groups()]
    assert group_phases == [0.0, math.pi, 0.0]
    group_durations = [amp.duration for amp, _, _ in composite_drive._split_in_phase_groups()]
...
def test_drive_phase_split_with_equal_constant_amplitudes() -> None:
    ...
    groups = (drive_1 >> drive_2)._split_in_phase_groups()
