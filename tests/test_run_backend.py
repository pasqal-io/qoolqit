...
from pulser_pasqal.backends import RemoteEmulatorBackend, EmuFreeBackendV2, EmuMPSBackend
...
from qoolqit.execution import CompilerProfile, RemoteEmulator, LocalEmulator
...
@pytest.mark.parametrize("backend_type", [QutipBackendV2, SVBackend, MPSBackend,])
@pytest.mark.parametrize("runs", [500, 100])
def test_local_backend_run(
    device_class: Callable, profile: CompilerProfile, random_program: Callable[[], QuantumProgram], backend_type: type[EmulatorBackend], runs: int,
) -> None:
    ...
    if profile != CompilerProfile.MIN_DISTANCE:
        ...
        assert isinstance(tuple, results)
